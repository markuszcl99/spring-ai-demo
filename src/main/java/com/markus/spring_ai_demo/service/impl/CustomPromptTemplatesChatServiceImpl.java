 ...
 */
@Service("customPromptTemplatesChatServiceImpl")
public class CustomPromptTemplatesChatServiceImpl extends AbstractChatServiceImpl {
    @Override
    public String chat(String modelId, String prompt) throws Exception {
        ChatModel chatModel = getModelService().getChatModel(modelId);
        ActorFilms actorFilms = ChatClient.builder(chatModel)
                .build()
                .prompt()
